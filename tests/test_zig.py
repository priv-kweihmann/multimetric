...
class TestClassZig():
    ...
    def test_zig(self):
        file = os.path.join(pytest.test_dir_local, 'test.zig')

        res, _, _ = self._run(file)

        assert 'Zig' in res.get('files', {}).get(file, {}).get('lang', [])
        assert res.get('files', {}).get(file, {}).get('comment_ratio', 0) == 12.0
        ...
        assert res.get('files', {}).get(file, {}).get('halstead_bugprop', 0) == 0.085
        assert res.get('files', {}).get(file, {}).get('halstead_difficulty', 0) == 9.75
        assert res.get('files', {}).get(file, {}).get('halstead_effort', 0) == 2483.856
        ...
        assert res.get('files', {}).get(file, {}).get('operators_uniq', 0) == 9
