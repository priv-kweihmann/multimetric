...
class TestClassJavaScript():
    ...
    def test_javascript(self):
        file = os.path.join(pytest.test_dir_local, 'test.js')

        res, _, _ = self._run(file)

        assert 'JavaScript' in res.get('files', {}).get(file, {}).get('lang', [])
        assert res.get('files', {}).get(file, {}).get('comment_ratio', 0) == 3.392
        assert res.get('files', {}).get(file, {}).get('cyclomatic_complexity', 0) == 5
        assert res.get('files', {}).get(file, {}).get('fanout_external', 0) == 4
        assert res.get('files', {}).get(file, {}).get('fanout_internal', 0) == 0
        assert res.get('files', {}).get(file, {}).get('halstead_bugprop', 0) == 0.253
        assert res.get('files', {}).get(file, {}).get('halstead_difficulty', 0) == 15.536
        assert res.get('files', {}).get(file, {}).get('halstead_effort', 0) == 11802.126
