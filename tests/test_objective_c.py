...
class TestClassObjectiveC():
    ...
    def test_objectivec(self):
        file = os.path.join(pytest.test_dir_local, 'test.m')

        res, _, _ = self._run(file)

        assert 'Objective-C' in res.get('files', {}).get(file, {}).get('lang', [])
        assert res.get('files', {}).get(file, {}).get('comment_ratio', 0) == 11.643
        ...
        assert res.get('files', {}).get(file, {}).get('halstead_bugprop', 0) == 0.766
        assert res.get('files', {}).get(file, {}).get('halstead_difficulty', 0) == 31.404
        assert res.get('files', {}).get(file, {}).get('halstead_effort', 0) == 72179.671
        ...
        assert res.get('files', {}).get(file, {}).get('operators_uniq', 0) == 23
