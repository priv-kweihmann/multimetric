...
class TestClassPerl():
    ...
    def test_perl(self):
        file = os.path.join(pytest.test_dir_local, 'test.pl')

        res, _, _ = self._run(file)

        assert 'Prolog' in res.get('files', {}).get(file, {}).get('lang', [])
        assert res.get('files', {}).get(file, {}).get('comment_ratio', 0) == 33.708
        ...
        assert res.get('files', {}).get(file, {}).get('halstead_bugprop', 0) == 0.035
        assert res.get('files', {}).get(file, {}).get('halstead_difficulty', 0) == 4.0
        assert res.get('files', {}).get(file, {}).get('halstead_effort', 0) == 421.944
