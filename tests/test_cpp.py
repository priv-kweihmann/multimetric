...
class TestClassCPP():
    ...
    def test_cpp(self):
        file = os.path.join(pytest.test_dir_local, 'test.cpp')

        res, _, _ = self._run(file)

        assert 'C++' in res.get('files', {}).get(file, {}).get('lang', [])
        assert res.get('files', {}).get(file, {}).get('comment_ratio', 0) == 4.805
        ...
        assert res.get('files', {}).get(file, {}).get('halstead_bugprop', 0) == 0.329
        assert res.get('files', {}).get(file, {}).get('halstead_difficulty', 0) == 44.571
        assert res.get('files', {}).get(file, {}).get('halstead_effort', 0) == 43960.132
        ...
        assert res.get('files', {}).get(file, {}).get('operators_uniq', 0) == 16
        ...
        assert res.get('files', {}).get(file, {}).get('tiobe_complexity', 0) == 12.673
