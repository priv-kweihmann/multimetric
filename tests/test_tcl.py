...
class TestClassTcl():
    ...
    def test_tcl(self):
        file = os.path.join(pytest.test_dir_local, 'test.tcl')

        res, _, _ = self._run(file)

        assert 'Tcl' in res.get('files', {}).get(file, {}).get('lang', [])
        assert res.get('files', {}).get(file, {}).get('comment_ratio', 0) == 7.042
        ...
        assert res.get('files', {}).get(file, {}).get('halstead_bugprop', 0) == 0.023
        assert res.get('files', {}).get(file, {}).get('halstead_difficulty', 0) == 1.333
        assert res.get('files', {}).get(file, {}).get('halstead_effort', 0) == 91.377
        ...
        assert res.get('files', {}).get(file, {}).get('operators_uniq', 0) == 2
