...
class TestClassGo():
    ...
    def test_c(self):
        file = os.path.join(pytest.test_dir_local, 'test.go')

        res, _, _ = self._run(file)

        assert 'Go' in res.get('files', {}).get(file, {}).get('lang', [])
        assert res.get('files', {}).get(file, {}).get('comment_ratio', 0) == 13.608
        ...
        assert res.get('files', {}).get(file, {}).get('halstead_bugprop', 0) == 0.159
        assert res.get('files', {}).get(file, {}).get('halstead_difficulty', 0) == 20.0
        assert res.get('files', {}).get(file, {}).get('halstead_effort', 0) == 9518.563
